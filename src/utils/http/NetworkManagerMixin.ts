  ...
  return <M>(target: any, propertyKey: string | symbol, description: TypedPropertyDescriptor<M>): TypedPropertyDescriptor<M> => {
    ...
    const newMethod = (async function (this: NetworkManagerMixin, ...args: any[]) {
      ...
      try {
        ...
        this.$set(this.loading, propertyKey as string, false);
        this.$set(this.error, propertyKey as string, (response as IJsonResponse).errors!.summary || summary);
        this.$set(this.failed, propertyKey as string, (response as IJsonResponse).errors!.summary || summary);
        this.errorFields = (response as IJsonResponse).errors!.fields || fields;
